...
// Neue, noch unveröffentlichte Stellenangebote Dashboard Widget
function obj_new_jobs_dashboard_widget() {
	query_posts( 'post_type=stellenangebote&post_status=draft');
	if (have_posts()) :
?>
	<ul class="job-liste">
		...
		<li id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> >
			<h4>
				<a href="post.php?post=<?php the_ID(); ?>&action=edit" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a>
...
<?php
		...
		if($eingang != "") {
			foreach ( $eingang as $key => $value ) {
				printf('Eingang: ' . $value);
			}
		} else {
			printf('Ohne Eingangsdatum');
		}
?>
				</span>
			</h4>
	...
	<p class="textright"><a href="edit.php?post_status=draft&post_type=stellenangebote">Alle ansehen</a></p>
	<?php else : ?>
	<p><?php _e("Es gibt nichts zu tun, geh Kaffee trinken.", "bonestheme"); ?></p>
	<?php endif; ?>
	<?php wp_reset_postdata(); ?>
<?php
}
//Abgelaufene Stellenangebote
function obj_expired_jobs_dashboard_widget() {
...
<?php 
		...
		$custom_fields = get_post_custom();
		$eingang = $custom_fields['Eingang'];
		$frist = $custom_fields['Bewerbungsfrist'];
		//nur wenn ein Feld Bewerbungsfrist vorhanden ist
		//var_dump(isset($frist));
		if(isset($frist) && $frist != "") {
			foreach ( $frist as $key => $value ) {
				//ermitteln ob keine
				$pos = strpos($value, "keine");
				if($pos === false && $value != "") {
					$frist2 = date("Y.m.d", strtotime($value));
					$heute = date("Y.m.d");
					if($frist2 < $heute) {
						++$abgelaufen;
?>
					<li id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> >
						<h4>
							<a href="post.php?post=<?php the_ID(); ?>&action=edit" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a>
							<span style="color: #999;">
								<?php echo ("Bewerbungsfrist: " . $value); ?>
							</span>
						</h4>
					</li>
<?php
...
?>
		...
		<?php if($abgelaufen === 0) { ?>
		<p><?php _e("Alle Stellenangebote sind aktuell.", "bonestheme"); ?></p>
		<?php } ?>
	<?php else : ?>
	<p><?php _e("Keine veröffentlichte Angebote gefunden.", "bonestheme"); ?></p>
	<?php endif; ?>
<?php wp_reset_postdata(); 
}
